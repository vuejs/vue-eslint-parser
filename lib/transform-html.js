 ...
 */
function isVForVariable(leafNode) { //eslint-disable-line complexity
    let node = leafNode
    ...
    while (parent != null && node.type !== "VExpressionContainer") {
        switch (parent.type) {
            ...
            case "SpreadElement":
            case "RestElement":
            case "SpreadProperty":             // ※ Legacy.
            case "ExperimentalSpreadProperty": // ※ Legacy.
            case "RestProperty":               // ※ Legacy.
            case "ExperimentalRestProperty":   // ※ Legacy.
                break
            ...
            case "BinaryExpression":
                if (parent.operator !== "in" || parent.left !== node) {
                    return false
                }
                break
...
function extractScopeVariables(references, variables) {
    while (references.length >= 1) {
        ...
        variables.push({id: reference.id})
    }
}
...
class HTMLTransformer {
    ...
    createVExpressionContainer(parent, start, end, quoteSize) {
        ...
        try {
            const parsed = this.scriptParser.parseExpression(
                start + quoteSize,
                end - quoteSize,
                this.tokenGenerator
            )
